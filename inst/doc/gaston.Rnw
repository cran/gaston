...
\section*{Introduction}
  ...
  To do so, it is sufficient to create a file \verb!~/.R/Makevars! containing
  \verb!CXX = clang++! 
...
\subsection*{Modifying Gaston's behaviour with global options}
...
@
  ...
  \verb!hz! it the heterozygosity rate.
...
\end{center}
  ...
  allowing to update only some slots. The following code illustrates this:
...
\subsection{Genetic Relationship Matrix and Principal Component Analysis}
  ...
  \textbf{NOTE:} In the current version (1.4.5) of Gaston, \verb!GRM! uses all SNPs in the computation.
  The user should extract a bed.matrix with only autosomal SNPs befaore calling GRM. This will
