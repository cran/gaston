...
// [[Rcpp::export]]
LogicalVector ld_thin_left(XPtr<matrix4> pA, NumericVector mu, NumericVector sd, double threshold, 
    ...
    while(j <= end && pos(j) < max_pos && chr(j) == chr_i) {
      ...
      if(w(j-beg)) {
        ...
        if(fabs(ld) > threshold) {
          if(debug) std::cout << " removing snp " << j;
          w(j-beg) = false; 
        }
        else
          if(!gotnexti) {
...
RcppExport SEXP gg_ld_thin_left(SEXP pASEXP, SEXP muSEXP, SEXP sdSEXP, SEXP thresholdSEXP, SEXP posSEXP, SEXP chrSEXP, SEXP max_distSEXP, SEXP begSEXP, SEXP endSEXP) {
BEGIN_RCPP
    SEXP __sexp_result;
    {
        ...
        LogicalVector __result = ld_thin_left(pA, mu, sd, threshold, pos, chr, max_dist, beg, end);
